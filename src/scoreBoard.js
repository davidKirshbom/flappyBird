let scoreBoard = {
  position: { x: gameCanvas.width * 0.5, y: gameCanvas.height * 0.2 },
  score: 0,
...
scoreBoard.setNumberImages = function () {
  for (let i = 0; i <= 9; i++) {
    ...
    scoreBoard[i].src = `../numbers/${i}.png`;
  }
};
...
scoreBoard.update = function (tubes) {
  if (
    tubes.some((tubesLine) => {
      if (
        tubesLine.upperTube.position.x < bird.positionX &&
...
scoreBoard.boardImage.src = "../FinishScoreBoard.png";
